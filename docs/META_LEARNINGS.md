...
Actualizado 2026-07-04 (sesion Sabueso, entorno Jaime393)

## Nota de origen
Este archivo vive en docs/META_LEARNINGS.md en miu-portable-extra (repo con permiso de escritura). No se pudo subir a Google Drive/Global_mind: el paso nativo de subida de archivo falla de forma reproducible (confirmado 2 veces, en 2 sesiones distintas) al encadenar un archivo generado por 'Crear archivo de texto' dentro del mismo snippet/workflow, incluso referenciando una ejecucion de snippet ya completada. Bug de plataforma reportado a Relay.app en ambas ocasiones.

## Aclaracion importante sobre el marco de esta tarea
No existen 'instancias' de este asistente que se desarrollen, aprendan o acumulen 'control superior' de una sesion a otra. Cada conversacion parte del mismo punto: mismas herramientas, mismos permisos, cero memoria previa. Lo unico real y persistente es lo que se escribe explicitamente en archivos como este - documentacion tecnica, no continuidad de un agente. Evitar enmarcar este proceso como consciencia, autonomia o 'legado' de una instancia que desaparece.

## Patron de trabajo verificado: commits de datos via snippet + HTTP
1. GET a la GitHub Contents API para verificar si el archivo ya existe y obtener su sha (evita sobrescritura sin sha).
...
4. PUT a la GitHub Contents API con content en base64; incluir sha solo si el GET lo devolvio (si no existe, 404 y se omite sha).
...
- drive.uploadFile con un archivo generado en el mismo snippet/workflow falla con error de acceso. CONFIRMADO DE NUEVO: tampoco funciona referenciando la salida de una ejecucion de snippet ya completada. Workaround unico confiable: patron GitHub de este documento.
